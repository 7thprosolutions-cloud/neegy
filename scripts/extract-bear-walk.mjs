...
const s3 = JSON.parse(fs.readFileSync("assets/sprites/s3_manifest.json"));
const walk1 = s3.find((f) => f.name === "s3_r0_c0");
const merged = s3.find((f) => f.name === "s3_r0_c1"); // WALK2..WALK8

(async () => {
  // WALK1: already isolated cleanly, just re-key it through the same pipeline for consistency
  await extractClean("Sprite/Sprite 3.png", walk1.x - 4, walk1.y - 4, walk1.x + walk1.w + 4, walk1.y + walk1.h + 4, "assets/sprites/enemy/walk1.png");

  const sliceCount = 7;
  const sliceW = merged.w / sliceCount;
  for (let i = 0; i < sliceCount; i++) {
    const rx1 = Math.round(merged.x + i * sliceW);
    const rx2 = Math.round(merged.x + (i + 1) * sliceW);
    const ry1 = Math.max(0, merged.y - 10);
    const ry2 = merged.y + merged.h + 10;
    await extractClean("Sprite/Sprite 3.png", rx1, ry1, rx2, ry2, `assets/sprites/enemy/walk${i + 2}.png`);
  }
})();
