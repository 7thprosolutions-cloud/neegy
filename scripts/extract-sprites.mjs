...
const OUT_DIR = path.resolve("assets/sprites");
const DEBUG_DIR = path.resolve("scripts/_debug");
fs.mkdirSync(OUT_DIR, { recursive: true });
fs.mkdirSync(DEBUG_DIR, { recursive: true });

const WHITE_LUMA = 246; // pixels with luma >= this are treated as background
const MIN_AREA = 6000; // discard small blobs (text glyphs, dividers)
const PAD = 6;
...
function findComponents(data, width, height) {
  ...
  for (let i = 0; i < width * height; i++) {
    ...
    ink[i] = luma(r, g, b) < WHITE_LUMA ? 1 : 0;
  }

  // strip long *contiguous, thin* straight rule lines (row separators, column
  // dividers) without touching character strokes, which are filled solid
  // (thick locally) even where their outline is thin.
    ...
    for (let x = 0; x <= width; x++) {
      ...
      if (v === 1) {
        ...
        if (runLen / width > 0.5) {
          ...
          const thickness = verticalInkHeightAt(mid, y);
          if (thickness <= 5) {
  ...
  for (let x = 0; x < width; x++) {
    ...
    for (let y = 0; y <= height; y++) {
      ...
      if (v === 1) {
        ...
        if (runLen > 250) {
          const mid = runStart + Math.floor(runLen / 2);
          const thickness = horizontalInkWidthAt(x, mid);
          if (thickness <= 5) {
...
function clusterRows(components) {
  // group by vertical center proximity to assign row order, then sort left-to-right
...
async function processSheet(file, prefix) {
  ...
  for (let r = 0; r < rows.length; r++) {
    for (let c = 0; c < rows[r].items.length; c++) {
      ...
      const { data: cropData, info } = await sharp(file)
        .ensureAlpha()
        .extract({ left: x1, top: y1, width: w, height: h })
        .raw()
        .toBuffer({ resolveWithObject: true });

      // key out near-white background with soft falloff
...
const files = [
  ["Sprite/Sprite 1.png", "s1"],
  ["Sprite/Sprite 2.png", "s2"],
  ["Sprite/Sprite 3.png", "s3"],
];
